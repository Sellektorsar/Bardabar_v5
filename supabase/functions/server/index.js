...
import logger from 'npm:hono@4.4.12/logger';
...
app.get('/make-server-c85ae302/reservations', async (c) => {
    try {
        ...
        const allBookings = [...reservations, ...bookings].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
...
// Получить настройки
app.get('/make-server-c85ae302/settings', async (c) => {
    try {
...
// Обновить настройки
app.patch('/make-server-c85ae302/settings', async (c) => {
    try {
        const body = await c.req.json();
        await kv.set('settings:general', body);
        return c.json({ success: true });
    }
    catch (error) {
