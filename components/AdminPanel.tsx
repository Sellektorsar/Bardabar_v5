...
import { Users, Menu, Settings, Plus, Edit, Trash2, Save, Bell } from 'lucide-react'
...
export function AdminPanel() {
  ...
  const addMenuItem = () => {
    if (newMenuItem.name && newMenuItem.price) {
      ...
      setNewMenuItem({ name: '', description: '', price: '', category: 'Горячие блюда', image: '', isSpecial: false })
  ...
  return (
    <section className="container mx-auto px-4 py-16">
      ...
      <Tabs defaultValue="notifications" className="w-full">
        <TabsList className="grid w-full grid-cols-4 mb-12 bg-white/80 backdrop-blur-sm border-2 border-orange-200/50 rounded-2xl p-2 shadow-xl">
          <TabsTrigger 
            value="notifications" 
        ...
        <TabsContent value="staff" className="space-y-6">
          ...
          <Card className="border-0 shadow-2xl hover:shadow-3xl transition-all duration-500 bg-white/90 backdrop-blur-sm rounded-3xl overflow-hidden">
            ...
            <CardContent className="space-y-4">
              {staff.map((member) => (
                <div key={member.id} className="flex items-center justify-between p-6 border-0 rounded-2xl bg-gradient-to-br from-orange-50 to-red-50 hover:shadow-xl transition-all duration-500 transform hover:scale-[1.02]">
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      ...
                      <Badge variant="secondary" className="bg-gradient-to-r from-orange-500 to-red-600 text-white border-0 rounded-full px-3 py-1">{member.position}</Badge>
                    </div>
                  </div>
        ...
        <TabsContent value="menu" className="space-y-6">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex items-center space-x-2">
                <Switch
                  ...
                  onCheckedChange={(checked) => setNewMenuItem({...newMenuItem, isSpecial: checked})}
                />
          ...
          <Card className="border-orange-200 shadow-lg hover:shadow-xl transition-shadow duration-300">
            ...
            <CardContent className="space-y-4">
              {menuItems.map((item) => (
                <div key={item.id} className="flex items-center justify-between p-4 border border-orange-100 rounded-xl bg-gradient-to-r from-orange-50/50 to-red-50/50 hover:shadow-md transition-all duration-300">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex items-center space-x-2">
                  <Switch
                    ...
                    onCheckedChange={(checked) => setEditingMenuItem({...editingMenuItem, isSpecial: checked})}
                  />
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">Разрешить пользователям бронировать столики</p>
                  </div>
                  <Switch
                    checked={settings.acceptsReservations}
                    onCheckedChange={(checked) => setSettings({...settings, acceptsReservations: checked})}
                  />
                </div>
              </div>
